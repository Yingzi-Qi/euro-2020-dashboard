(() => {
  ...
  const source=window.EURO_RESULTS;
  if(!d3||!source?.outcomes){$('#trend-chart').innerHTML='<div class="empty-chart">Charts could not load. Please reconnect and refresh.</div>';return;}
  ...
  function dataset(kind){const q=source.outcomes[kind];return {
    daily:q.daily.map(r=>({...r,x:new Date(r.date+'T00:00:00Z'),with:r.fitted_total,without:r.background})),
    matches:q.matches.map((r,i)=>({...r,index:i,x:new Date(r.date+'T00:00:00Z'),baseline:r.B,dow:r.D,match:r.M,total:r.fitted_total,ratio:r.match_background_ratio,flags:flags[i]}))
  };}
  const outcomes={contacts:dataset('contacts'),transmissions:dataset('transmissions')};
  let {daily,matches}=outcomes.contacts;
  const label=()=>state.outcome==='transmissions'?'Transmissions':'Contacts';
  ...
  let state={outcome:'contacts',compareOrder:'date',separateScales:false,view:'daily',period:'full',start:0,end:daily.length-1,selected:6,order:'date',theme:'dark',visible:{with:true,without:true}};
  try{const saved=JSON.parse(localStorage.getItem('euro-dashboard-count-scale'));if(saved){if(['light','dark'].includes(saved.theme))state.theme=saved.theme;if(['daily','cumulative'].includes(saved.view))state.view=saved.view;if(Number.isInteger(saved.selected)&&saved.selected>=0&&saved.selected<8)state.selected=saved.selected;}}
  catch(e){}
  ...
  function persist(){try{localStorage.setItem('euro-dashboard-count-scale',JSON.stringify({theme:state.theme,view:state.view,selected:state.selected}));}catch(e){}}
  ...
  function rows(){let a=0,b=0,c=0;return daily.slice(state.start,state.end+1).map(d=>({...d,cumWith:(a+=d.with),cumWithout:(b+=d.without),cumObserved:(c+=d.observed)}));}
  function val(d,k){return state.view==='daily'?d[k]:d[k==='with'?'cumWith':k==='without'?'cumWithout':'cumObserved'];}
  ...
  function sync(){
    ...
    $$('[data-outcome]').forEach(b=>{const active=b.dataset.outcome===state.outcome;b.setAttribute('aria-selected',String(active));b.tabIndex=active?0:-1;});
    $('#single-outcome').hidden=state.outcome==='compare';$('#compare-outcomes').hidden=state.outcome!=='compare';
    $('#single-outcome').setAttribute('aria-labelledby','tab-'+(state.outcome==='transmissions'?'transmissions':'contacts'));
    $('#trend-title').textContent=label()+' trajectory';
    $('.component-line.total span').textContent='Total fitted '+label().toLowerCase();
    $('#table-title').textContent=state.outcome==='compare'?'Compare exact match data':label()+' match data';
    $('.bottom-bar span').textContent='Count-scale refit · '+(state.outcome==='compare'?'Both outcomes':label())+' · 2021';
    $('#compare-window').textContent=shortDate(daily[state.start].x)+' – '+shortDate(daily[state.end].x)+' 2021';
    drawTable();
  }
  function updateMetrics(animate){const data=rows(),w=d3.sum(data,d=>d.with),n=d3.sum(data,d=>d.without);number('#total-with',w,whole,animate);number('#total-without',n,whole,animate);number('#total-gap',w-n,whole,animate);number('#reduction',1-n/w,n=>percent(n)+' of total',animate);
  ...
  function drawTrend(animate=true){
    ...
    const x=d3.scaleUtc().domain(d3.extent(data,d=>d.x)).range([l+3,r-3]),y=d3.scaleLinear().domain([0,d3.max(data.flatMap(d=>[val(d,'with'),val(d,'without'),val(d,'observed')]))*1.17]).nice().range([b,t]);trendX=x;
    s.selectAll('title').data([0]).join('title').text('Observed counts, fitted total B+D+M, and dashed background B+D. Selected match dates are linked to the match explorer.');
    ...
    s.selectAll('text.measure-label').data([0]).join('text').attr('class','svg-label measure-label').attr('x',l).attr('y',12).text((state.view==='daily'?'Daily ':'Cumulative ')+label().toLowerCase());
    ...
    layer.selectAll('circle.observed-point').data(data,d=>d.date).join('circle').attr('class','observed-point').attr('r',2.5).attr('fill','var(--text)').attr('opacity',.75).attr('cx',d=>x(d.x)).attr('cy',d=>y(val(d,'observed')));
    ...
    function show(e){const px=d3.pointer(e,s.node())[0],j=d3.bisector(d=>d.x).center(data,x.invert(px)),d=data[j];guide.attr('x1',x(d.x)).attr('x2',x(d.x)).style('visibility','visible');hover.attr('cx',x(d.x)).attr('cy',k=>y(val(d,k))).style('visibility',k=>state.visible[k]?'visible':'hidden');let html='<div class="tip-title">'+longDate(d.x)+' 2021</div>';for(const k of ['with','without'])if(state.visible[k])html+=tipRow(k==='with'?'Fitted total':'Background',format(val(d,k)));html+=tipRow('Observed',format(val(d,'observed')));if(state.visible.with&&state.visible.without)html+=tipRow('Fixed match component',format(val(d,'with')-val(d,'without')),true);const match=matches.find(m=>m.date===d.date);if(match)html+=tipRow('Match',match.name,true);tooltip(e,html);}
  ...
  function setRange(a,b,period,animate=true){state.start=Math.max(0,a);state.end=Math.min(daily.length-1,b);state.period=period;sync();if(state.outcome==='compare'){drawPaired();}else{updateMetrics(animate);drawTrend(animate);moveBrush();}hideTip();}
  function updateSpotlight(animate=true){const m=selected();$('#match-counter').textContent='Match '+String(m.index+1).padStart(2,'0')+' / 08';$('#selected-flags').innerHTML=img(m.flags[0],m.name.split(' vs ')[0])+'<span>vs</span>'+img(m.flags[1],m.name.split(' vs ')[1]);$('#selected-name').textContent=m.name;$('#selected-date').textContent=longDate(m.x)+' 2021 · '+dayName(m.x);number('#selected-ratio',m.ratio,n=>d3.format('.3f')(n)+'×',animate);number('#selected-share',m.match/m.total,d3.format('.1%'),animate);number('#selected-baseline',m.baseline,format,animate);number('#selected-dow',m.dow,d3.format('+,.2f'),animate);number('#selected-match',m.match,format,animate);number('#selected-total',m.total,format,animate);const length=2*Math.PI*43;trans(d3.select('#share-arc'),animate).attr('stroke-dasharray',(m.match/m.total*length)+' '+length);
    $('#share-arc').parentElement.setAttribute('aria-label','Match component is '+percent(m.match/m.total)+' of fitted daily counts.');
  ...
  function drawComparison(animate=true){const container=d3.select('#comparison-body');const old=new Map([...container.node().children].map(n=>[n.dataset.index,n.getBoundingClientRect().top]));
    ...
    groups.select('.row-ratio').html(d=>d3.format('.3f')(d.ratio)+'×'+'<span><i style="width:'+(d.ratio/d3.max(matches,m=>m.ratio)*100)+'%"></i></span>');
  ...
  function drawScatter(animate=true){const {chart:s,width:w}=svg('#scatter-chart',367),l=54,r=w-24,t=35,b=309;
    ...
    s.selectAll('title').data([0]).join('title').text('Each point is one match. The horizontal axis is fitted background counts. The vertical axis is estimated match-component counts. The dashed equality line has match counts equal to background counts, so their ratio is one. Each point’s vertical value divided by its horizontal value is the match-to-background ratio.');
    ...
    s.selectAll('text.y-title').data([0]).join('text').attr('class','svg-label y-title').attr('x',l).attr('y',17).text('Match '+label().toLowerCase());s.selectAll('text.x-title').data([0]).join('text').attr('class','svg-label x-title').attr('text-anchor','middle').attr('x',(l+r)/2).attr('y',354).text('Background '+label().toLowerCase());
  ...
  function playTour(){if(tourTimer){stopTour();return;}setRange(daily.findIndex(d=>d.date==='2021-06-11'),daily.findIndex(d=>d.date==='2021-07-11'),'euro',true);selectMatch(0);$('#play-tour span').textContent='Pause tour';$('#play-tour').setAttribute('aria-pressed','true');let index=0;tourTimer=setInterval(()=>{index++;if(index>=matches.length){stopTour();return;}selectMatch(index);},2100);}
  function tableRows(){return (state.outcome==='compare'?['contacts','transmissions']:[state.outcome]).flatMap(k=>outcomes[k].matches.filter(m=>m.x>=daily[state.start].x&&m.x<=daily[state.end].x).map(m=>({...m,outcome:k})));}
  function drawTable(){$('#data-table').innerHTML=tableRows().map(m=>'<tr><td>'+shortDate(m.x)+'</td><td>'+(state.outcome==='compare'?m.outcome+' · ':'')+m.name+'</td><td>'+format(m.baseline)+'</td><td>'+d3.format('+,.2f')(m.dow)+'</td><td>'+format(m.match)+'</td><td>'+format(m.total)+'</td><td>'+d3.format('.3f')(m.ratio)+'×</td></tr>').join('');}
  function exportData(){const records=tableRows().map(m=>({date:m.date,outcome:m.outcome,match:m.name,baseline_B:m.baseline,weekday_D:m.dow,background:m.background,match_M:m.match,fitted_total:m.total,observed:m.observed,match_background_ratio:m.ratio}));const a=document.createElement('a'),url=URL.createObjectURL(new Blob([d3.csvFormat(records)+'\n'],{type:'text/csv;charset=utf-8;'}));a.href=url;a.download='match_'+state.outcome+'_count_scale.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  function pairedMatches(){return outcomes.contacts.matches.filter(m=>m.x>=daily[state.start].x&&m.x<=daily[state.end].x).map(m=>({date:m.date,name:m.name,x:m.x,flags:m.flags,contacts:m,transmissions:outcomes.transmissions.matches[m.index]})).sort((a,b)=>{if(state.compareOrder==='date')return a.x-b.x;const [k,v]=state.compareOrder.split('-');return b[k][v==='ratio'?'ratio':'match']-a[k][v==='ratio'?'ratio':'match'];});}
  function drawPaired(){
    const data=pairedMatches(),keys=['contacts','transmissions'];
    $('#compare-metrics').innerHTML=keys.map(k=>'<article><h3>'+ (k==='contacts'?'Contacts':'Transmissions')+'</h3><strong>'+format(d3.sum(data,m=>m[k].match))+'</strong><p>Estimated match-associated counts · '+data.length+' selected matches</p></article>').join('');
    $('#count-scale-note').textContent=state.separateScales?'Fixed M · separate scales: compare numbers, not bar lengths across outcomes':'Fixed M · both outcomes use the same count scale';
    for(const metric of ['ratio','count']){
      const field=metric==='ratio'?'ratio':'match',container=$('#'+metric+'-bars');
      if(!data.length){container.innerHTML='<p class="empty-chart">No selected matches in this date window.</p>';continue;}
      const common=d3.max(data.flatMap(m=>keys.map(k=>m[k][field])))||1;
      const maximum=Object.fromEntries(keys.map(k=>[k,metric==='count'&&state.separateScales?(d3.max(data,m=>m[k][field])||1):common]));
      container.innerHTML=data.map(m=>'<div class="paired-row"><div class="paired-identity">'+m.flags[2]+'–'+m.flags[3]+'<span>'+shortDate(m.x)+'</span></div><div class="paired-series">'+keys.map(k=>'<div class="paired-value '+k+'" tabindex="0" aria-label="'+m.name+', '+k+', '+(metric==='ratio'?'match-to-background ratio ':'match counts ')+(metric==='ratio'?d3.format('.3f')(m[k][field])+' times':format(m[k][field]))+'"><div class="paired-track" aria-hidden="true"><div class="paired-fill" style="width:'+100*m[k][field]/maximum[k]+'%"></div></div><strong>'+(metric==='ratio'?d3.format('.3f')(m[k][field])+'×':format(m[k][field]))+'</strong></div>').join('')+'</div></div>').join('');
    }
  }
  function setOutcome(kind){stopTour();hideTip();state.outcome=kind;if(kind!=='compare')({daily,matches}=outcomes[kind]);render(false);$('#status').textContent=kind==='compare'?'Comparing contact and transmission ratios and match counts.':label()+' selected.';}
  $$('[data-outcome]').forEach((b,i)=>{b.addEventListener('click',()=>setOutcome(b.dataset.outcome));b.addEventListener('keydown',e=>{const tabs=$$('[data-outcome]');let next;if(e.key==='ArrowRight')next=(i+1)%3;if(e.key==='ArrowLeft')next=(i+2)%3;if(e.key==='Home')next=0;if(e.key==='End')next=2;if(next!==undefined){e.preventDefault();tabs[next].focus();setOutcome(tabs[next].dataset.outcome);}});});
  $('#compare-order').addEventListener('change',e=>{state.compareOrder=e.target.value;drawPaired();});
  $('#separate-scales').addEventListener('change',e=>{state.separateScales=e.target.checked;drawPaired();});
  ...
  $('#period').addEventListener('change',e=>{stopTour();const p=e.target.value;if(p==='custom')return;const a=p==='full'?0:daily.findIndex(d=>d.date===(p==='matches'?'2021-06-13':'2021-06-11')),b=p==='full'?daily.length-1:daily.findIndex(d=>d.date==='2021-07-11');setRange(a,b,p);});
  ...
  function render(animate=false){sync();if(state.outcome==='compare'){drawPaired();return;}updateMetrics(animate);drawTrend(animate);drawTimeline();drawComparison(animate);drawScatter(animate);updateSpotlight(animate);}
